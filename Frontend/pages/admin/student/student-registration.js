...
const studentRegistration = ({ classes, departments }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await axios.post(
        `/api/revalidateUser?role=student&id=${response.data.id}`
      );
            ...
            <div className="w-full mb-6 group">
              ...
              >
                ...
                {classes.map((class_, key) => {
                  return <option key={key}>{class_.className}</option>;
                })}
              </select>
            </div>
...
export async function getStaticProps(context) {
  ...
  const response2 = await axios.get(
    `${process.env.NEXT_PUBLIC_API_URL}/api/class/getAll`
  );
