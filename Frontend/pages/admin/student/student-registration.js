...
const studentRegistration = ({ classes, departments }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await axios.post(`/api/revalidateUser?role=student&id=${response.data.id}`);
        ...
        <div className='relative text-slate-700 my-8 w-5/12 mx-auto rounded-xl shadow-xl border border-blue-700'>
          ...
          <form className='w-[80%] mx-auto px-2' onSubmit={handleSubmit}>
            ...
            <div className="w-full mb-6 group">
              ...
              <input type="number" name="rollNo" id="rollNo" className="p-2 border-b-2 block focus:border-blue-500 focus:outline-none w-full" placeholder='eg. 42335' onChange={handleInputChange} value={formData.rollNo} required />
            </div>

            <button className='px-2 py-3 bg-indigo-600 w-full rounded-lg text-white mb-12'>Create Student Account</button>
          </form>
