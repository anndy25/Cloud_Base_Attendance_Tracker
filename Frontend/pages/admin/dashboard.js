      ...
      <div className="min-h-screen flex justify-center">
        ...
        <div className="w-[82%]">
          ...
          <section className="w-11/12 min-h-screen mx-auto flex flex-col items-center">
            <h1 className="font-bold text-2xl text-gray-600 my-6 w-full px-4">
              Dashboard
            </h1>
                  ...
                  <span
                    className={`px-6 py-3 font-semibold border-b-2 ${
                      tab === 1
                        ...
                        : "text-gray-500 hover:text-indigo-600  hover:border-indigo-600"
                    }  cursor-pointer `}
                    onClick={() => setTab(1)}
                  >
                    Students
...
export async function getServerSideProps(context) {
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/api/overview/admin`
    );
